...
def generate_html_report(
    ...
    if is_daily_summary:
        try:
            ...
            frontend_data = {
                "meta": {
                    "type": "当日汇总" if mode != "current" else "当前榜单",
                    "total": total_titles,
                    "hot": sum(len(stat["titles"]) for stat in report_data.get("stats", [])),
            ...
            for idx, stat in enumerate(stats_list, 1):
                group = {
                    ...
                    "name": stat["word"],
                    "count": stat["count"],
                    "isHot": stat["count"] >= 10, # 简单判定热度
                    "index": f"{idx:02d}/{total_groups:02d}",
                    "items": []
                }
                
                for item_idx, title_data in enumerate(stat["titles"], 1):
                    # 处理排名显示
                    ranks = title_data.get("ranks", [])
                    if ranks:
                        rank_display = str(min(ranks)) if min(ranks) == max(ranks) else f"{min(ranks)}-{max(ranks)}"
                    else:
                        rank_display = "-"
                    ...
                    group["items"].append({
                        ...
                        "rank": rank_display,
                        "time": title_data.get("time_display", "").replace(" ~ ", "~").replace("[", "").replace("]", ""),
                        "views": f"{title_data['count']}次" if title_data['count'] > 1 else "",
                        "title": title_data["title"],
                        "url": title_data.get("mobile_url") or title_data.get("url", ""),
                        "isNew": title_data.get("is_new", False)
                    })
                
                frontend_data["groups"].append(group)
            
            # 处理新增新闻 (如果有)
            if report_data.get("new_titles"):
                new_group_idx = total_groups + 1
                new_group = {
                    "id": new_group_idx,
                    "name": "NEW_ARRIVALS // 新增热点",
                    "count": report_data.get("total_new_count", 0),
                    "isHot": True,
                    "index": f"{new_group_idx:02d}/{new_group_idx:02d}", # 临时逻辑
                    "items": []
                }
                ...
                for source_data in report_data["new_titles"]:
                    for title_data in source_data["titles"]:
                         ranks = title_data.get("ranks", [])
                         rank_display = str(min(ranks)) if ranks else "?"
                         
                         new_group["items"].append({
                            "id": f"new-{item_counter}",
                ...
                frontend_data["groups"].append(new_group)
